<template>
  <div class="header">
    ...
    <div class="bulletin-wrapper">
      <img class="icon" v-if="poinInfo.discounts2" :src="poinInfo.discounts2[0].icon_url" />

      <span class="text" v-if="poinInfo.discounts2">{{poinInfo.discounts2[0].info}}</span>
      <div class="detail" v-if="poinInfo.discounts2" @click="showBulletin">
        {{poinInfo.discounts2.length}}个活动
      ...
      <div class="bulletin-detail" v-show="isShow">
        <div class="detail-wrapper">
          ...
          <div class="main-wrapper" :style="detail_bg">
            ...
            <!-- 星级评价 -->
            <div class="score">
              <app-star :score="poinInfo.wm_poi_score"></app-star>
              <span>{{poinInfo.wm_poi_score}}</span>
            </div>

            <p class="tip">
              {{poinInfo.min_price_tip}}
              <i>|</i>
              {{poinInfo.shipping_fee_tip}}
              <i>|</i>
              {{poinInfo.delivery_time_tip}}
            </p>

            <p class="time">配送时间: {{poinInfo.shipping_time}}</p>

            <div class="discounts" v-if="poinInfo.discounts2">
              <p>
                <img :src="poinInfo.discounts2[0].icon_url" />
                <span>{{poinInfo.discounts2[0].info}}</span>
              </p>
            </div>
          </div>
...
export default {
  ...
  computed: {
    head_pic_url() {
      return "background-image: url(" + this.poinInfo.head_pic_url + ")";
    },
    head_bg() {
      return "background-image: url(" + this.poinInfo.pic_url + ")";
    },
    detail_bg() {
      return "background-image: url(" + this.poinInfo.poi_back_pic_url + ")";
    }
  },
  methods: {
    showBulletin() {
